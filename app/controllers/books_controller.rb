...
class BooksController < ApplicationController
  ...
  def show
    ...
    @quotes = if sort == 'random'
                Quote.where(book_id: params[:id]).where(source_quote_id: nil).order('RANDOM()').page(params[:page])
              else
                Quote.where(book_id: params[:id]).where(source_quote_id: nil).order(created_at: :desc).page(params[:page])
              end
